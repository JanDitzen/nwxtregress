...
mata:
	function SparseDefine(real matrix mat, real matrix uniquer)
	{
		timer_on(21)
...
mata:
	...
	{
		if (issparse == 1) {
			uniid = uniqrows(idt[.,1])
			N = rows(uniid)
			
			result = J(N,N,0)


			uniqrows = uniqrows(sparse[.,1])
			Nuniqrows = rows(uniqrows)
			uniqrowsC = uniqrows(sparse[.,2])

			i = 1
			while (i<=Nuniqrows) {				
				ii = uniqrows[i]				
				index = selectindex(sparse[.,1]:==ii)
				
				col_i = sparse[index,2]
				submat = sparse[index,3]
				j = 1
				...
				while (j<=rows(col_i)) {
					...
					pos_col = selectindex(uniqrowsC:==jj)
					
					if (sum(pos_col) > 0) {
						result[i,pos_col] = submat[j]
					}
					j++
				}
				i++
